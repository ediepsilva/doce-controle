...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$clienteValido) {
        header('Location: pedidos.php?erro=cliente');
        exit;
    }

    if (!$receita) {
        header('Location: pedidos.php?erro=receita');
        exit;
    }

    if ($quantidade <= 0) {
        header('Location: pedidos.php?erro=quantidade');
        exit;
    }

    if (empty($data_entrega)) {
        header('Location: pedidos.php?erro=data');
        exit;
    }
